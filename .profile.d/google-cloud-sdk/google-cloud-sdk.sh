...
if command -v brew > /dev/null 2>/dev/null; then
    _SDK_PYTHON="$(brew --prefix)/bin/python3.12"
elif command -v asdf >/dev/null 2>/dev/null; then
    _SDK_PYTHON="$(asdf which python3.12)"
elif command -v pyenv >/dev/null 2>/dev/null; then
    _SDK_PYTHON="$(pyenv prefix "$(pyenv whence python3.12 | sort -nr | head -n 1)")/bin/python"
else
    _SDK_PYTHON="$(which python3.12 2>/dev/null)"
fi
